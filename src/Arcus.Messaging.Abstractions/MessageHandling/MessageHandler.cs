...
namespace Arcus.Messaging.Abstractions.MessageHandling
{
    ...
    public class MessageHandler
    {
        ...
        public object GetMessageHandlerInstance()
        {
            ...
            {
                const string propertyName = nameof(MessageHandlerRegistration<object, MessageContext>.Service);
                object messageHandlerType = _service.GetRequiredPropertyValue(propertyName, BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            where TMessageContext : MessageContext
        {
            ...
            const string methodName = nameof(MessageHandlerRegistration<object, TMessageContext>.CanProcessMessageWithinMessageContext);
            var canProcessMessage = (bool) _service.InvokeMethod(methodName, BindingFlags.Instance | BindingFlags.NonPublic, messageContext);
        ...
        public bool CanProcessMessageBasedOnMessage(object? message)
        {
            if (_service.GetType().Name == typeof(MessageHandlerRegistration<,>).Name)
            {
                ...
                const string methodName = nameof(MessageHandlerRegistration<object, MessageContext>.CanProcessMessageBasedOnMessage);
                var canProcessMessage = (bool) _service.InvokeMethod(methodName, BindingFlags.Instance | BindingFlags.NonPublic, message);
        ...
        public async Task<MessageResult> TryCustomDeserializeMessageAsync(string message)
        {
            if (_service.GetType().Name == typeof(MessageHandlerRegistration<,>).Name)
            {
                ...
                const string propertyName = nameof(MessageHandlerRegistration<object, MessageContext>.Serializer);
                var serializer = _service.GetPropertyValue<IMessageBodySerializer>(propertyName, BindingFlags.NonPublic | BindingFlags.Instance);
                if (serializer != null)
                {
            ...
            CancellationToken cancellationToken) where TMessageContext : MessageContext
        {
            ...
            const string methodName = nameof(IMessageHandler<object, TMessageContext>.ProcessMessageAsync);
            try
            {
